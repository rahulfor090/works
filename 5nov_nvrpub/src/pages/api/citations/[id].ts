...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === 'GET') {
      const [rows] = await query(`SELECT id, title, url, logo, isPublished, createdAt, updatedAt FROM citations WHERE id = ?`, [id])
    ...
    if (req.method === 'PUT') {
      ...
      await query(
        `UPDATE citations SET title = ?, url = ?, logo = ?, isPublished = ?, updatedAt = NOW() WHERE id = ?`,
        [title, url, logo ?? '', isPublished ? 1 : 0, id]
      )
