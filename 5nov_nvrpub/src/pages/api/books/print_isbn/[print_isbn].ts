...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { print_isbn } = req.query
  const printIsbnStr = String(print_isbn || '').trim()
  if (!printIsbnStr) {
    return res.status(400).json({ message: 'Invalid print_isbn' })
  }
  ...
  try {
    ...
    if (req.method === 'GET') {
      const [books]: any = await query(
        'SELECT id, isbn, print_isbn, title, author, coverImage, description, keywords, subjectcategoryId, rating, ratingCount FROM books WHERE print_isbn = ? OR isbn = ? LIMIT 1',
        [printIsbnStr, printIsbnStr]
      )
      const book = books?.[0]
      if (!book) {
