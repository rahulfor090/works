  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Link href={`/contenttypes/${slug}/${item.id}`}>
            <Box sx={{ display: 'inline-block' }}>
              <Image src={item.coverImage || item.cover || '/images/placeholder.jpg'} width={234} height={326} alt={'Content ' + item.id} />
            </Box>
          </Link>
        </Box>
        <Box sx={{ mb: 2 }}>
          <Link href={`/contenttypes/${slug}/${item.id}`} style={{ textDecoration: 'none' }}>
